...
int main(void) {
 ...
 for (int i=0; i<dyld_image_count; i++) {
  ...
  if (strcmp(name,"posix_spawn") != 0) { //check if image name is posix_spawn
    rebind_symbols_image(_dyld_get_image_header(index), _dyld_get_image_vmaddr_slide(index), (struct rebinding[1]){{"posix_spawn", hook_posix_spawn, NULL }}, 1);
  }
 }
}
